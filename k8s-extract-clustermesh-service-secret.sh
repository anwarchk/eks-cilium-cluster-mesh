...
SERVICE_NAME=$(kubectl -n $NAMESPACE get svc clustermesh-apiserver -o jsonpath='{.status.loadBalancer.ingress[0].hostname}')
SERVICE_PORT=$(kubectl -n $NAMESPACE get svc clustermesh-apiserver -o jsonpath='{.spec.ports[0].port}')
...
ETCD_CONFIG=`cat <<EOF |
endpoints:
- https://${SERVICE_NAME}:${SERVICE_PORT}
